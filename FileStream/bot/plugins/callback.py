...
async def gen_file_menu(_id, file_list_no, update: CallbackQuery):
    ...
    # تبدیل تاریخ‌ها به زمان ایران
    tz_iran = pytz.timezone('Asia/Tehran')
    
    # تاریخ ایجاد به شمسی
    create_dt = datetime.datetime.fromtimestamp(create_time, tz_iran)
    create_jalali = jdatetime.fromgregorian(datetime=create_dt)
    create_date = create_jalali.strftime('%Y/%m/%d - %H:%M:%S')
    
    # تاریخ انقضا به شمسی
    expire_dt = datetime.datetime.fromtimestamp(expire_time, tz_iran)
    expire_jalali = jdatetime.fromgregorian(datetime=expire_dt)
    expire_date = expire_jalali.strftime('%Y/%m/%d - %H:%M:%S')
